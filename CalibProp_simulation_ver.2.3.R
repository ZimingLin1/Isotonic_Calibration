...
sim_one_ATE_est_with_pi <- function(K_times, n_samples, overlap=1, outcome_reg_type, ps_est_type, calibration_method){
  ...
  return(list(bias=bias, std=std, mse=mse, cvrg_prob=cvrg_prob,
              AIPW_est=AIPW_est, lower_conf_bd=lower_conf_bd, upper_conf_bd=upper_conf_bd,
              true_ATE=true_ATE))  
}
...
sim_one_ATE_est_with_alpha <- function(K_times, n_samples, overlap=1, outcome_reg_type, alpha_est_type){
  ...
  for(k in 1:K_times){
    dat <- DGP(n_sample=n_samples, overlap=overlap)
    AIPW_est[k] <- get_ATE_AIPW_with_alpha(dat, outcome_reg_type, alpha_est_type)
  }
  ...
  std <- sd(AIPW_est)
  mse <- bias^2+std^2
  confint <- quantile(AIPW_est, c(0.025, 0.975))
  
  return(list(bias=bias, std=std, mse=mse, confint=confint))  
}
...
sim_ATE_est_with_pi_n_varying <- function(K_times, n_samples_range = seq(1000, 11000, by=2000), overlap=1, outcome_reg_type, ps_est_type){
